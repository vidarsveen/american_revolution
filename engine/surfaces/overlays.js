...
export function unmountOverlays() {
  root?.remove();
  root = null;
  portraitEl = imageEl = quoteEl = statsEl = noteEl = compareEl = factEl = null;
  entries = null;
  factBeat = null;
}
...
export default {
  id: 'overlays',
  // The cards, over everything the stage draws. .stage-overlays is z-index 30
  // in css/story.css: above the plate (20) so a quote still reads over a
  // picture, below the caption slot and the transport.
  layer: 30,
  verbs: {
    'portrait.show': (c, i) => showPortrait(c, i),
    'portrait.hide': ()     => hidePortrait(),

    'image.show':    (c, i) => showImage(c, i),
    'image.hide':    ()     => hideImage(),

    'quote.show':    (c, i) => showQuote(c, i),
    'quote.hide':    ()     => hideQuote(),

    'stat.show':     (c, i) => showStat(c, i),
    'stat.clear':    ()     => clearStats(),

    // Two or three numbers as one picture, rather than as unrelated chips.
    'compare.show':  (c, i) => showCompare(c, i),
    'compare.clear': ()     => clearCompare(),

    'fact.show':     (c, i) => showFact(c, i),
    'fact.hide':     ()     => hideFact(),

    'caption.note':  (c, i) => showNote(c, i),
  },
  mount(container, ch, ctx = {}) {
    mountOverlays(container, ch, ctx.people || [], ctx.lang);
    // fact.show resolves against every declared kind, not four fixed pools.
    useEntries(ctx.entries || null);
    return null;
  },
  reset() { resetOverlays(); },
  unmount() { unmountOverlays(); },
};
